      ...
      try {
        ...
        if (accessToken && refreshToken) {
          ...
          if (data.session?.user) {
            console.log('[Xentory Bet] SSO login OK:', data.session.user.email);
            setUser(sbUserToNexus(data.session.user));
            setLoading(false);
            return;
          }
          if (error) console.warn('[Xentory Bet] setSession error:', error.message);
        }
        ...
        setLoading(false);
        setTimeout(() => { window.location.href = HUB_URL + '?redirect=bet'; }, 300);
      } catch (e) {
        console.error('[Xentory Bet] auth init error:', e);
        setLoading(false);
        setTimeout(() => { window.location.href = HUB_URL + '?redirect=bet'; }, 300);
      }
    }
