...
function mapFixtureToMatch(f: any): Match {
  const comp = COMPETITIONS.find((c: any) => c.id === f.league?.id) ?? { id: f.league?.id ?? 0, name: f.league?.name ?? 'Liga', sport: 'football' as const, country: f.league?.country ?? '', logo: f.league?.logo ?? '', emoji: '⚽' };
  return { id: f.fixture.id, sport: 'football', competition: comp, homeTeam: { id: f.teams.home.id, name: f.teams.home.name, shortName: f.teams.home.name.slice(0,3).toUpperCase(), logo: f.teams.home.logo }, awayTeam: { id: f.teams.away.id, name: f.teams.away.name, shortName: f.teams.away.name.slice(0,3).toUpperCase(), logo: f.teams.away.logo }, date: f.fixture.date, status: mapStatus(f.fixture.status.short), homeScore: f.goals.home ?? undefined, awayScore: f.goals.away ?? undefined, venue: f.fixture.venue?.name, round: f.league?.round };
}
...
function mapApiStatsToTeamStats(stats: any, fixtures: any[], teamId: number): TeamStats {
  ...
  return { team: { id: teamId, name: stats?.team?.name ?? 'Equipo', shortName: (stats?.team?.name ?? 'EQP').slice(0,3).toUpperCase(), logo: stats?.team?.logo }, form, goalsScored: parseFloat(gs), goalsConceded: parseFloat(gc), cleanSheets: stats?.clean_sheet?.total ?? 0, btts: 50, over25: 55, possession: stats?.ball_possession ? parseInt(stats.ball_possession) : undefined, shotsOnTarget: stats?.shots?.on?.total ?? undefined, homeRecord: { w: stats?.fixtures?.wins?.home ?? 5, d: stats?.fixtures?.draws?.home ?? 2, l: stats?.fixtures?.loses?.home ?? 2 }, awayRecord: { w: stats?.fixtures?.wins?.away ?? 3, d: stats?.fixtures?.draws?.away ?? 3, l: stats?.fixtures?.loses?.away ?? 3 } };
}
...
const MOCK_TEAMS = [
  ...
  { id: 530, name: 'Atlético Madrid', shortName: 'ATL' }, { id: 40, name: 'Liverpool', shortName: 'LIV' },
  { id: 505, name: 'Inter Milan', shortName: 'INT' }, { id: 165, name: 'Borussia Dortmund', shortName: 'BVB' },
  { id: 489, name: 'AC Milan', shortName: 'MIL' }, { id: 33, name: 'Manchester United', shortName: 'MUN' },
];
...
function getMockMatches(leagueId: number, limit: number): Match[] {
  ...
  return Array.from({ length: Math.min(limit, 4) }, (_, i) => {
    ...
    return { id: 10000 + leagueId * 10 + i, sport: 'football' as const, competition: comp, homeTeam: { id: home.id, name: home.name, shortName: home.shortName }, awayTeam: { id: away.id, name: away.name, shortName: away.shortName }, date: new Date(now + (i + 1) * 2 * 86400000).toISOString(), status: 'scheduled' as const, round: `Jornada ${28 + i}`, venue: `Estadio ${home.name}` };
  });
}
...
function getMockTeamStats(teamId: number): TeamStats {
  ...
  const now = Date.now();
  const form: FormMatch[] = results.map((result, i) => ({ opponent: opponents[i]?.name ?? 'Rival', result, goalsFor: result === 'W' ? 2 : result === 'D' ? 1 : 0, goalsAgainst: result === 'L' ? 2 : result === 'D' ? 1 : 0, date: new Date(now - (i + 1) * 7 * 86400000).toISOString(), isHome: i % 2 === 0 }));
  return { team: { id: team.id, name: team.name, shortName: team.shortName }, form, goalsScored: 1.8 + Math.random() * 0.8, goalsConceded: 0.9 + Math.random() * 0.6, cleanSheets: 8, btts: 50, over25: 55, possession: 55, shotsOnTarget: 6, homeRecord: { w: 7, d: 2, l: 1 }, awayRecord: { w: 5, d: 3, l: 2 } };
}
...
export async function fetchTennisMatches(): Promise<Match[]> {
  ...
  const [raw, raw2] = await Promise.all([
    callSportsAPI('https://v1.tennis.api-sports.io', `/games?date=${todayStr}`),
    callSportsAPI('https://v1.tennis.api-sports.io', `/games?date=${nextWeekStr}`),
  ]);
  const combined = [...(raw ?? []), ...(raw2 ?? [])];
  if (!combined.length) return getMockMatchesBySport('tennis');
  const matches: Match[] = combined.filter((g: any) => g?.status?.short === 'NS').slice(0, 12).map((g: any, idx: number) => ({
    id: g.id ?? 30000 + idx, sport: 'tennis' as const,
    competition: { id: g.tournament?.id ?? idx, name: g.tournament?.name ?? 'ATP/WTA', sport: 'tennis', country: g.country?.name ?? 'Internacional', logo: '', emoji: '🎾' },
    homeTeam: { id: g.players?.home?.id ?? 20000 + idx * 2, name: formatPlayerName(g.players?.home?.name ?? 'Jugador 1'), shortName: (g.players?.home?.name ?? 'J1').split(' ').map((w: string) => w[0]).join('').toUpperCase().slice(0,3) },
    awayTeam: { id: g.players?.away?.id ?? 20001 + idx * 2, name: formatPlayerName(g.players?.away?.name ?? 'Jugador 2'), shortName: (g.players?.away?.name ?? 'J2').split(' ').map((w: string) => w[0]).join('').toUpperCase().slice(0,3) },
    date: g.date ?? new Date(Date.now() + 86400000).toISOString(), status: 'scheduled', venue: g.venue ?? g.tournament?.name,
  }));
...
export async function fetchBasketballMatches(): Promise<Match[]> {
  ...
  const matches: Match[] = raw.filter((g: any) => g?.status?.short === 'NS').slice(0, 12).map((g: any, idx: number) => ({
    id: g.id ?? 20000 + idx, sport: 'basketball' as const,
    competition: { id: g.league?.id ?? 12, name: g.league?.name ?? 'NBA', sport: 'basketball', country: g.country?.name ?? 'USA', logo: '', emoji: '🏀' },
    ...
    date: g.date ?? new Date(Date.now() + 86400000).toISOString(), status: 'scheduled', venue: g.arena?.name,
  }));
...
export async function fetchF1Matches(): Promise<Match[]> {
  ...
  return upcoming.map((r: any, idx: number) => ({
    ...
    competition: { id: 50, name: `Formula 1 ${season}`, sport: 'f1', country: 'Mundial', logo: '', emoji: '🏎️' },
    homeTeam: { id: 300 + idx * 2, name: r.circuit?.name ?? `GP ${r.competition?.location?.country ?? ''}`, shortName: 'GP' },
    awayTeam: { id: 300 + idx * 2 + 1, name: r.competition?.location?.city ?? 'Grand Prix', shortName: 'F1' },
    date: r.date ?? new Date(Date.now() + 86400000 * (idx + 1)).toISOString(), status: 'scheduled', venue: r.circuit?.name,
  }));
}
...
export function getMockMatchesBySport(sport: string): Match[] {
  ...
  if (sport === 'f1') return [
    { id: 4001, sport: 'f1', competition: { id: 50, name: 'Formula 1 2025', sport: 'f1', country: 'Mundial', logo: '', emoji: '🏎️' }, homeTeam: { id: 301, name: 'Max Verstappen (Red Bull)', shortName: 'VER' }, awayTeam: { id: 302, name: 'Lewis Hamilton (Ferrari)', shortName: 'HAM' }, date: new Date(now + 3 * 86400000).toISOString(), status: 'scheduled', venue: 'GP Baréin' },
    { id: 4002, sport: 'f1', competition: { id: 50, name: 'Formula 1 2025', sport: 'f1', country: 'Mundial', logo: '', emoji: '🏎️' }, homeTeam: { id: 303, name: 'Charles Leclerc (Ferrari)', shortName: 'LEC' }, awayTeam: { id: 304, name: 'Lando Norris (McLaren)', shortName: 'NOR' }, date: new Date(now + 3 * 86400000).toISOString(), status: 'scheduled', venue: 'GP Baréin' },
  ];
...
export function getMockStatsBySport(teamId: number, teamName: string, sport: string): TeamStats {
  ...
  const opponents = sport === 'tennis' ? ['Medvedev', 'Zverev', 'Rublev', 'Fritz', 'Tsitsipas']
    : sport === 'basketball' ? ['Lakers', 'Celtics', 'Heat', 'Nuggets', 'Bucks']
    : sport === 'f1' ? ['Verstappen', 'Hamilton', 'Leclerc', 'Norris', 'Sainz']
    : ['Oponente A', 'Oponente B', 'Oponente C', 'Oponente D', 'Oponente E'];
  const now = Date.now();
  const form: FormMatch[] = results.map((result, i) => ({ opponent: opponents[i], result, goalsFor: result === 'W' ? 2 : 1, goalsAgainst: result === 'L' ? 2 : 1, date: new Date(now - (i + 1) * 7 * 86400000).toISOString(), isHome: i % 2 === 0 }));
  ...
  return { team: { id: teamId, name: teamName, shortName }, form, goalsScored: scored, goalsConceded: conceded, cleanSheets: 0, btts: 50, over25: 55, possession: undefined, shotsOnTarget: undefined, homeRecord: { w: 8, d: 0, l: 2 }, awayRecord: { w: 6, d: 0, l: 4 } };
}
