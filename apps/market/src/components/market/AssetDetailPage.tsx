...
function IndicatorBadge({ label, value, sub, highlight }: { label: string; value: string; sub?: string; highlight?: boolean }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: '1.1rem', color: highlight ? 'var(--gold)' : 'var(--text)' }}>{value}</div>
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div className="glass-2" style={{ padding: '0.7rem 1rem', borderRadius: 10, fontSize: '0.8rem' }}>
      ...
      <div style={{ fontFamily: 'Syne', fontWeight: 700, color: 'var(--gold)' }}>{payload[0].value.toFixed(4)}</div>
    </div>
  );
};
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.4rem' }}>
              <span style={{ color: isUp ? 'var(--green)' : 'var(--red)', fontFamily: 'Syne', fontWeight: 700, fontSize: '1rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{
                ...
                <div>
                  ...
                  <div style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: '1rem', color: SIGNAL_LABELS[analysis.signal]?.color }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: '1.2rem', color: SIGNAL_LABELS[analysis.signal]?.color }}>
                    {analysis.confidence}%
                  </div>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.6rem' }}>
                <div style={{ padding: '0.7rem 0.9rem', background: 'rgba(0,255,136,0.05)', border: '1px solid rgba(0,255,136,0.1)', borderRadius: 8 }}>
                  ...
                  {analysis.keyLevels.support.map((s, i) => (
                    <div key={i} style={{ fontFamily: 'Syne', fontWeight: 600, fontSize: '0.85rem' }}>{formatPrice(s, asset.category)}</div>
                  ))}
                </div>
                <div style={{ padding: '0.7rem 0.9rem', background: 'rgba(255,68,85,0.05)', border: '1px solid rgba(255,68,85,0.1)', borderRadius: 8 }}>
                  ...
                  {analysis.keyLevels.resistance.map((r, i) => (
                    <div key={i} style={{ fontFamily: 'Syne', fontWeight: 600, fontSize: '0.85rem' }}>{formatPrice(r, asset.category)}</div>
                  ))}
                </div>
              </div>
